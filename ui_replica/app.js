...
function setupDateInputs() {
    ...
    dateInputs.forEach(input => {
        ...
        // Prevent selecting weekends and ensure future dates
        input.addEventListener('change', (e) => {
            ...
            if (selectedDate < today) {
                showError('Cannot select past dates. Please select a future date.');
                e.target.value = '';
                return;
            }
            ...
            loadSlots(); // Reload slots when date changes
        });
    });
}
...
async function loadSlots() {
    ...
    if (day === 0 || day === 6) {
        slotsContainer.innerHTML = '<div class="message-error">Weekends are not available for bookings.</div>';
        return;
    }
    ...
    today.setHours(0, 0, 0, 0);
    if (selectedDate < today) {
        slotsContainer.innerHTML = '<div class="message-error">Cannot view slots for past dates.</div>';
        return;
    }
...
async function loadBookingHistory() {
    ...
    if (!authToken) {
        bookingsList.innerHTML = '<div class="message-info">Please log in to view your booking history.</div>';
        return;
    }
...
function displayBookingHistory(bookings) {
    ...
    if (!bookings || bookings.length === 0) {
        bookingsList.innerHTML = '<div class="empty-state"><p>You don\'t have any bookings yet.</p><p>Select a game and date above to make your first booking!</p></div>';
        return;
    }
    ...
    bookings.forEach(booking => {
        ...
        const canCancel = !isPastBooking && !booking.slot.is_cancelled;

        const card = document.createElement('div');
        card.className = 'booking-card' + (booking.slot.is_cancelled ? ' cancelled' : '');
        
        let cardContent = '<h3>' + booking.game.name + ' - ' + booking.game.type + '</h3>';
        cardContent += '<p>Date: ' + formattedDate + '</p>';
        cardContent += '<p>Time: ' + formattedStartTime + ' - ' + formattedEndTime + '</p>';
        
        const status = booking.slot.is_cancelled ? 'Cancelled' : 
                      isPastBooking ? 'Completed' : 'Active';
        cardContent += '<p>Status: ' + status + '</p>';
        
        if (canCancel) {
            cardContent += '<button class="cancel-button" onclick="handleCancelBooking(' + booking.id + ')">Cancel Booking</button>';
        }
...
function showGameForm() {
    if (!isAdmin) {
        showError('Only admins can manage games');
        return;
    }
...
async function handleCreateGame() {
    ...
    const maxPlayers = parseInt(document.getElementById('newGameMaxPlayers').value, 10);
    ...
    if (!maxPlayers || maxPlayers <= 0) {
        showError('Please provide a valid number of maximum players (greater than 0)');
        return;
    }
    ...
    try {
        ...
        document.getElementById('newGameMaxPlayers').value = '';
        // Refresh game lists
        await loadGames();
        loadAdminGames();
        ...
        showError('Failed to create game: ' + (err.message || err.detail || 'Unknown error'));
    }
}
